...
def main(args):
    ...
    if args.whether_load_model:
        model.load_state_dict(
            torch.load(args.model_save_path + str(args.start_epoch) + args.model_name)
        )
        print(
            'Model : "'
    ...
    for epoch in range(args.start_epoch + 1, args.start_epoch + 1 + args.num_epochs):
        ...
        for train_iter, train_data_batch in enumerate(tk0):
            ...
            train_images = torch.squeeze(
                torch.cat(
                    (
                        train_images.unsqueeze(-1),
                        torch.zeros_like(train_images.unsqueeze(-1)),
                    ),
                    dim=-1,
                ),
                dim=1,
            ...
            tk0.set_description_str(
                "Epoch {}/{} : Training".format(
                    epoch, args.start_epoch + 1 + args.num_epochs - 1
                )
            )
            tk0.set_postfix(
                {
        ...
        with torch.no_grad():
            ...
            for val_iter, val_data_batch in enumerate(tk1):
                ...
                val_images = torch.squeeze(
                    torch.cat(
                        (
                            val_images.unsqueeze(-1),
                            torch.zeros_like(val_images.unsqueeze(-1)),
                        ),
                        dim=-1,
                    ),
                    dim=1,
                ...
                tk1.set_postfix(
                    {
                        ...
                        "Val_Accuarcy": "{:.5f}".format(val_accuracy),
                    }
                )
        ...
        torch.save(
            model.state_dict(), (args.model_save_path + str(epoch) + args.model_name)
        )
        print(
            'Model : "'
            + args.model_save_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--whether-load-model", type=bool, default=False, help="是否加载模型继续训练"
    )
    parser.add_argument(
        "--start-epoch", type=int, default=0, help="从哪个epoch继续训练"
    )
    # 数据和模型相关
