...
def main(args):
    ...
    if args.whether_load_model:
        model.load_state_dict(torch.load(
            args.model_save_path + str(args.start_epoch) + args.model_name))
        print('Model : "' + args.model_save_path +
              str(args.start_epoch) + args.model_name + '" loaded.')
    else:
    ...
    for epoch in range(args.start_epoch + 1, args.start_epoch + 1 + args.num_epochs):
        ...
        tk0 = tqdm(train_dataloader, ncols=100,
        ...
        for train_iter, train_data_batch in enumerate(tk0):
            ...
            train_counter_ = torch.eq(torch.argmax(train_labels, dim=1), torch.argmax(
                train_outputs, dim=1)).float().sum()
            ...
            tk0.set_description_str(
                'Epoch {}/{} : Training'.format(epoch, args.start_epoch + 1 + args.num_epochs - 1))
            tk0.set_postfix({'Train_Loss': '{:.5f}'.format(
        ...
        with torch.no_grad():
            ...
            for val_iter, val_data_batch in enumerate(tk1):
                ...
                tk1.set_description_str(
                    'Epoch {}/{} : Validating'.format(epoch, args.start_epoch + 1 + args.num_epochs - 1))
                tk1.set_postfix({'Val_Loss': '{:.5f}'.format(
        ...
        torch.save(model.state_dict(),
                   (args.model_save_path + str(epoch) + args.model_name))
        print('Model : "' + args.model_save_path +
              str(epoch) + args.model_name + '" saved.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--whether-load-model', type=bool,
                        default=False, help="是否加载模型继续训练")
    parser.add_argument('--start-epoch', type=int,
                        default=0, help='从哪个epoch继续训练')
    # 数据和模型相关
    parser.add_argument('--model-name', type=str, default='_model.pth')
    parser.add_argument('--model-save-path', type=str,
                        default="./saved_model/")
    parser.add_argument('--result-record-path', type=pathlib.Path,
                        default="./result.csv", help="数值结果记录路径")
