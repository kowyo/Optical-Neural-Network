...
class DiffractiveLayer(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # distance bewteen two layers (3cm)
        self.distance = 0.03
        ...
        self.phi = np.fromfunction(
            lambda x, y: np.square(
                (x - (self.size // 2)) * self.fi) + np.square((y - (self.size // 2)) * self.fi),
            shape=(self.size, self.size), dtype=np.complex64)
        # h (200, 200)
        h = np.fft.fftshift(np.exp(1.0j * self.wn * self.distance) *
                            np.exp(-1.0j * self.wl * np.pi * self.distance * self.phi))
        # self.h (200, 200, 2)
        self.h = torch.nn.Parameter(torch.stack((torch.from_numpy(
            h.real), torch.from_numpy(h.imag)), dim=-1), requires_grad=False)

    def forward(self, waves):
        # waves (batch, 200, 200, 2)
        temp = torch.fft(waves, signal_ndim=2)
        k_pace_real = self.h[..., 0] * temp[..., 0] - \
            self.h[..., 1] * temp[..., 1]
        k_space_imag = self.h[..., 0] * \
            temp[..., 1] + self.h[..., 1] * temp[..., 0]
...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for index, layer in enumerate(self.diffractive_layers):
            temp = layer(x)
            exp_j_phase = torch.stack(
                (torch.cos(self.phase[index]), torch.sin(self.phase[index])), dim=-1)
            x_real = temp[..., 0] * exp_j_phase[..., 0] - \
                temp[..., 1] * exp_j_phase[..., 1]
            x_imag = temp[..., 0] * exp_j_phase[..., 1] + \
                temp[..., 1] * exp_j_phase[..., 0]
